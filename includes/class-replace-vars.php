...
use RANKMATH_SCHEMA\Traits\Replacement;
...
class Replace_Vars {
	...
	private function get_page() {
		...
		if ( $max > 1 && $page > 1 ) {
			...
			$replacement = sprintf( $sep . ' ' . __( 'Page %1$d of %2$d', 'schema-markup' ), $page, $max );
		}
	...
	public static function register_replacement( $var, $callback, $args = array() ) {
		...
		if ( false === preg_match( '`^[A-Z0-9_-]+$`i', $var ) ) {
			trigger_error( esc_html__( 'A replacement variable can only contain alphanumeric characters, an underscore or a dash. Try renaming your variable.', 'schema-markup' ), E_USER_WARNING );
			return false;
		}
		...
		if ( ! method_exists( __CLASS__, 'get_' . $var ) ) {
			if ( ! isset( self::$external_replacements[ $var ] ) ) {
				...
				trigger_error( esc_html__( 'A replacement variable with the same name has already been registered. Try making your variable name unique.', 'schema-markup' ), E_USER_WARNING );
			}
		} else {
			trigger_error( esc_html__( 'You cannot overrule a Rank Math standard variable replacement by registering a variable with the same name. Use the "wpseo_replacements" filter instead to adjust the replacement value.', 'schema-markup' ), E_USER_WARNING );
		}
	...
	public static function setup() {
		...
		if ( empty( self::$external_replacements ) ) {
			 ...
			 */
			do_action( 'rank_math_schema/vars/register_extra_replacements' );
		}
	}
	...
	public static function set_replacements() {
		...
		$post_id      = 1;
		$post_title   = esc_html__( 'Hello World', 'schema-markup' );
		...
		self::$replacements['title'] = array(
			'name'    => esc_html__( 'Post Title', 'schema-markup' ),
			'desc'    => esc_html__( 'Title of the current post/page', 'schema-markup' ),
			'example' => $post_title,
		);

		self::$replacements['parent_title'] = array(
			'name'    => esc_html__( 'Post Title of parent page', 'schema-markup' ),
			'desc'    => esc_html__( 'Title of the parent page of the current post/page', 'schema-markup' ),
			'example' => esc_html__( 'Example Title', 'schema-markup' ),
		);

		self::$replacements['sep'] = array(
			'name'    => esc_html__( 'Separator Character', 'schema-markup' ),
			'desc'    => esc_html__( 'Separator character, as set in the Title Settings', 'schema-markup' ),
			'example' => $self->get_sep(),
		);

		self::$replacements['sitename'] = array(
			'name'    => esc_html__( 'Site Title', 'schema-markup' ),
			'desc'    => esc_html__( 'Title of the site', 'schema-markup' ),
			'example' => get_bloginfo( 'name' ),
		);

		self::$replacements['sitedesc'] = array(
			'name'    => esc_html__( 'Site Description', 'schema-markup' ),
			'desc'    => esc_html__( 'Description of the site', 'schema-markup' ),
			'example' => get_bloginfo( 'description' ),
		);

		self::$replacements['date'] = array(
			'name'    => esc_html__( 'Date Published', 'schema-markup' ),
			'desc'    => wp_kses_post( __( 'Publication date of the current post/page <strong>OR</strong> specified date on date archives', 'schema-markup' ) ),
			'example' => current_time( get_option( 'date_format' ) ),
		);

		self::$replacements['modified'] = array(
			'name'    => esc_html__( 'Date Modified', 'schema-markup' ),
			'desc'    => esc_html__( 'Last modification date of the current post/page', 'schema-markup' ),
			'example' => current_time( get_option( 'date_format' ) ),
		);

		self::$replacements['excerpt'] = array(
			'name'    => esc_html__( 'Post Excerpt', 'schema-markup' ),
			'desc'    => esc_html__( 'Excerpt of the current post (or auto-generated if it does not exist)', 'schema-markup' ),
			'example' => esc_html__( 'Post Excerpt', 'schema-markup' ),
		);

		self::$replacements['excerpt_only'] = array(
			'name'    => esc_html__( 'Post Excerpt', 'schema-markup' ),
			'desc'    => esc_html__( 'Excerpt of the current post (without auto-generation)', 'schema-markup' ),
			'example' => esc_html__( 'Post Excerpt', 'schema-markup' ),
		);

		self::$replacements['tag'] = array(
			'name'    => esc_html__( 'Post Tag', 'schema-markup' ),
			'desc'    => wp_kses_post( __( 'First tag (alphabetically) associated to the current post <strong>OR</strong> current tag on tag archives', 'schema-markup' ) ),
			'example' => esc_html__( 'Example Tag', 'schema-markup' ),
		);

		self::$replacements['tags'] = array(
			'name'    => esc_html__( 'Post Tags', 'schema-markup' ),
			'desc'    => esc_html__( 'Comma-separated list of tags associated to the current post', 'schema-markup' ),
			'example' => esc_html__( 'Example Tag 1, Example Tag 2', 'schema-markup' ),
		);

		self::$replacements['category'] = array(
			'name'    => esc_html__( 'Post Category', 'schema-markup' ),
			'desc'    => wp_kses_post( __( 'First category (alphabetically) associated to the current post <strong>OR</strong> current category on category archives', 'schema-markup' ) ),
			'example' => esc_html__( 'Example Category', 'schema-markup' ),
		);

		self::$replacements['categories'] = array(
			'name'    => esc_html__( 'Post Categories', 'schema-markup' ),
			'desc'    => esc_html__( 'Comma-separated list of categories associated to the current post', 'schema-markup' ),
			'example' => esc_html__( 'Example Category 1, Example Category 2', 'schema-markup' ),
		);

		self::$replacements['term'] = array(
			'name'    => esc_html__( 'Current Term', 'schema-markup' ),
			'desc'    => esc_html__( 'Current term name', 'schema-markup' ),
			'example' => esc_html__( 'Example Term', 'schema-markup' ),
		);

		self::$replacements['term_description'] = array(
			'name'    => esc_html__( 'Term Description', 'schema-markup' ),
			'desc'    => esc_html__( 'Current term description', 'schema-markup' ),
			'example' => esc_html__( 'Example Term Description', 'schema-markup' ),
		);

		self::$replacements['search_query'] = array(
			'name'    => esc_html__( 'Search Query', 'schema-markup' ),
			'desc'    => esc_html__( 'Search query (only available on search results page)', 'schema-markup' ),
			'example' => esc_html__( 'example search', 'schema-markup' ),
		);

		self::$replacements['name'] = array(
			'name'    => esc_html__( 'Post Author', 'schema-markup' ),
			'desc'    => esc_html__( 'Display author\'s nicename of the current post, page or author archive.', 'schema-markup' ),
			'example' => $current_user->display_name,
		);

		self::$replacements['user_description'] = array(
			'name'    => esc_html__( 'Author Description', 'schema-markup' ),
			'desc'    => esc_html__( 'Author\'s biographical info of the current post, page or author archive.', 'schema-markup' ),
			'example' => get_the_author_meta( 'desc' ),
		);

		self::$replacements['filename'] = array(
			'name'    => esc_html__( 'File Name', 'schema-markup' ),
			'desc'    => esc_html__( 'File Name of the attachment', 'schema-markup' ),
			'example' => 'sunrise at Maldives',
		);

		// Advanced.
		self::$replacements['userid'] = array(
			'name'    => esc_html__( 'Author ID', 'schema-markup' ),
			'desc'    => esc_html__( 'Author\'s user id of the current post, page or author archive.', 'schema-markup' ),
			'example' => $current_user->ID,
		);

		self::$replacements['id'] = array(
			'name'    => esc_html__( 'Post ID', 'schema-markup' ),
			'desc'    => esc_html__( 'ID of the current post/page', 'schema-markup' ),
			'example' => $post_id,
		);

		self::$replacements['focuskw'] = array(
			'name'    => esc_html__( 'Focus Keyword', 'schema-markup' ),
			'desc'    => esc_html__( 'Focus Keyword of the current post', 'schema-markup' ),
			'example' => esc_html__( 'Focus Keyword', 'schema-markup' ),
		);

		self::$replacements['page'] = array(
			'name'    => esc_html__( 'Page', 'schema-markup' ),
			'desc'    => esc_html__( 'Page number with context (i.e. page 2 of 4). Only displayed on page 2 and above.', 'schema-markup' ),
			'example' => ' page 2 of 4',
		);

		self::$replacements['pagetotal'] = array(
			'name'    => esc_html__( 'Max Pages', 'schema-markup' ),
			'desc'    => esc_html__( 'Max pages number', 'schema-markup' ),
			'example' => '4',
		);

		self::$replacements['pagenumber'] = array(
			'name'    => esc_html__( 'Page Number', 'schema-markup' ),
			'desc'    => esc_html__( 'Current page number', 'schema-markup' ),
			'example' => '4',
		);

		self::$replacements['currenttime'] = array(
			'name'    => esc_html__( 'Current Time', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server time', 'schema-markup' ),
			'example' => current_time( get_option( 'time_format' ) ),
		);

		self::$replacements['currentdate'] = array(
			'name'    => esc_html__( 'Current Date', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server date', 'schema-markup' ),
			'example' => current_time( get_option( 'date_format' ) ),
		);

		self::$replacements['currentday'] = array(
			'name'    => esc_html__( 'Current Day', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server day', 'schema-markup' ),
			'example' => current_time( 'dS' ),
		);

		self::$replacements['currentmonth'] = array(
			'name'    => esc_html__( 'Current Month', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server month', 'schema-markup' ),
			'example' => current_time( 'F' ),
		);

		self::$replacements['currentyear'] = array(
			'name'    => esc_html__( 'Current Year', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server year', 'schema-markup' ),
			'example' => current_time( 'Y' ),
		);

		self::$replacements['pt_single'] = array(
			'name'    => esc_html__( 'Post Type Name Singular', 'schema-markup' ),
			'desc'    => esc_html__( 'Name of current post type (singular)', 'schema-markup' ),
			'example' => esc_html__( 'Product', 'schema-markup' ),
		);

		self::$replacements['pt_plural'] = array(
			'name'    => esc_html__( 'Post Type Name Plural', 'schema-markup' ),
			'desc'    => esc_html__( 'Name of current post type (plural)', 'schema-markup' ),
			'example' => esc_html__( 'Products', 'schema-markup' ),
		);

		self::$replacements['customfield(field-name)'] = array(
			'name'    => esc_html__( 'Custom Field (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Custom field value.', 'schema-markup' ),
			'example' => esc_html__( 'Custom field value', 'schema-markup' ),
		);

		self::$replacements['date(F jS, Y)'] = array(
			'name'    => esc_html__( 'Date Published (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Publish date with custom formatting pattern.', 'schema-markup' ),
			'example' => date( 'F jS, Y' ),
		);

		self::$replacements['modified(F jS, Y)'] = array(
			'name'    => esc_html__( 'Date Modified (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Modified date with custom formatting pattern.', 'schema-markup' ),
			'example' => date( 'F jS, Y' ),
		);

		self::$replacements['currenttime(F jS, Y)'] = array(
			'name'    => esc_html__( 'Current Time (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Current server time with custom formatting pattern.', 'schema-markup' ),
			'example' => current_time( 'F jS, Y' ),
		);

		self::$replacements['categories(limit=3&separator= | &exclude=12,23)'] = array(
			'name'    => esc_html__( 'Categories (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Output list of categories associated to the current post, with customization options.', 'schema-markup' ),
			'example' => esc_html__( 'Example Category 1 | Example Category 2', 'schema-markup' ),
		);

		self::$replacements['tags(limit=3&separator= | &exclude=12,23)'] = array(
			'name'    => esc_html__( 'Tags (advanced)', 'schema-markup' ),
			'desc'    => esc_html__( 'Output list of tags associated to the current post, with customization options.', 'schema-markup' ),
			'example' => esc_html__( 'Example Tag 1 | Example Tag 2', 'schema-markup' ),
		);

		self::$replacements['count(varname)'] = array(
			'name'    => esc_html__( 'Counter', 'schema-markup' ),
			'desc'    => esc_html__( 'Starts at 1 and increments by 1.', 'schema-markup' ),
			'example' => '2',
		);
	}
