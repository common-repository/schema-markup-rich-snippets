...
class Options {
	...
	function register_option_page() {
		...
		foreach ( $tabs as $id => $tab ) {
			...
			$cmb->add_field( array(
				'name' => esc_html__( 'Panel', 'schema-markup' ),
				'id'   => 'setting-panel-' . $id,
				'type' => 'tab_open',
			) );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'rank-math-schema-options', rank_math_schema()->plugin_url() . 'assets/admin/css/option-panel.css', array( 'select2-rm', 'rank-math-schema-common', 'rank-math-cmb2' ), rank_math_schema()->version );
		wp_enqueue_script( 'rank-math-schema-options', rank_math_schema()->plugin_url() . 'assets/admin/js/option-panel.js', array( 'underscore', 'select2-rm', 'rank-math-schema-common' ), rank_math_schema()->version, true );

		// Add thank you.
		Helper::add_json( 'indexUrl', rank_math_schema()->plugin_url() . 'assets/admin/js/search-index/' );
	...
	private function locate_file( $id, $tab ) {
		...
		$file = isset( $tab['file'] ) && ! empty( $tab['file'] ) ? $tab['file'] : rank_math_schema()->includes_dir() . "settings/{$this->folder}/{$id}.php";
