 ...
 * @subpackage RANKMATH_SCHEMA\Admin\Import
...
abstract class Plugin_Importer {
	...
	public function get_choices() {
		...
		$hash = array(
			'settings'     => esc_html__( 'Import Settings', 'schema-markup' ) . Admin_Helper::get_tooltip( esc_html__( 'Import Yoast plugin settings, global meta, sitemap settings, etc.', 'schema-markup' ) ),
			'postmeta'     => esc_html__( 'Import Post Meta', 'schema-markup' ) . Admin_Helper::get_tooltip( esc_html__( 'Import meta information of your posts/pages', 'schema-markup' ) ),
			'termmeta'     => esc_html__( 'Import Term Meta', 'schema-markup' ) . Admin_Helper::get_tooltip( esc_html__( 'Import data like category, tag, and CPT meta data from Yoast SEO.', 'schema-markup' ) ),
			'usermeta'     => esc_html__( 'Import Author Meta', 'schema-markup' ) . Admin_Helper::get_tooltip( esc_html__( 'Import User meta information., of your author archive pages.', 'schema-markup' ) ),
		);
	...
	public function run_import( $perform ) {
		...
		if ( ! method_exists( $this, $perform ) ) {
			throw new Exception( esc_html__( 'Unable to perform action this time.', 'schema-markup' ) );
		}
		...
		$hash_ok = array(
			'settings'     => esc_html__( 'Settings imported successfully.', 'schema-markup' ),
			'deactivate'   => esc_html__( 'Plugin deactivated successfully.', 'schema-markup' ),
			/* translators: start, end, total */
			'postmeta'     => esc_html__( 'Imported post meta for posts %1$s - %2$s out of %3$s ', 'schema-markup' ),
			/* translators: total */
			'termmeta'     => esc_html__( 'Imported term meta for %s terms.', 'schema-markup' ),
			/* translators: start, end, total */
			'usermeta'     => esc_html__( 'Imported user meta for users %1$s - %2$s out of %3$s ', 'schema-markup' ),
		);

		$hash_failed = array(
			'settings'     => esc_html__( 'Settings import failed.', 'schema-markup' ),
			'postmeta'     => esc_html__( 'Posts meta import failed.', 'schema-markup' ),
			'termmeta'     => esc_html__( 'Term meta import failed.', 'schema-markup' ),
			'usermeta'     => esc_html__( 'User meta import failed.', 'schema-markup' ),
		);
